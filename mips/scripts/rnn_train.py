...
def train(args):
    ...
    assert args.loss_fn in [
        "mse",
        "log",
        "cross_entropy",
    ], "loss must be one of 'mse', 'log', 'cross_entropy'"
    ...
    sequences_x_train, sequences_y_train, sequences_x_test, sequences_y_test = sequences

    if args.loss_fn == "cross_entropy":
        assert (
            args.output_dim == sequences_y_train.max() + 1
        ), "Number of classes in data doesn't match output dimension"
        assert (
            args.output_dim > 1
        ), "Output dimension must be greater than 1 for cross-entropy loss"
    ...
    if args.loss_fn == "cross_entropy":
        loss_fn = lambda y_pred, y_target: F.cross_entropy(
            y_pred, y_target, reduction="none"
        )
    elif args.loss_fn == "mse":
        loss_fn = lambda y_pred, y_target: 0.5 * torch.pow(y_pred - y_target, 2)
    elif args.loss_fn == "log":
        loss_fn = lambda y_pred, y_target: 0.5 * torch.log(
            1 + torch.pow(y_pred - y_target, 2)
        )
    ...
    for step in range(args.steps):
        if step % 100 == 0 or step == args.steps - 1:
            ...
            with torch.no_grad():
                ...
                x, y_target = prepare_xy(
                    x,
                    y_target,
                    args.vectorize_input,
                    args.input_dim,
                    dtype,
                    args.loss_fn,
                )
                ...
                if args.loss_fn == "cross_entropy":
                    y_pred = y_pred.transpose(
                        1, 2
                    )  # cross entropy expects (batch_size, num_classes, seq_len)
                ...
                if accuracy.item() == 1.0:
                    torch.save(
                        model.state_dict(),
                        os.path.join(args.save_dir, f"model_perfect.pt"),
                    )
        model.train()
        model.zero_grad()
        x, y_target = next(train_loader)
        x, y_target = prepare_xy(
            x, y_target, args.vectorize_input, args.input_dim, dtype, args.loss_fn
        )
        y_pred, _ = model.forward_sequence(x)
        if args.loss_fn == "cross_entropy":
            y_pred = y_pred.transpose(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hidden_dim", type=int, default=8, help="Hidden layer dimension"
    )
    parser.add_argument(
        "--hidden_mlp_depth", type=int, default=1, help="Depth of hidden MLP"
    )
    parser.add_argument(
        "--hidden_mlp_width", type=int, default=16, help="Width of hidden MLP"
    )
    parser.add_argument(
        "--output_mlp_depth", type=int, default=1, help="Depth of output MLP"
    )
    parser.add_argument(
        "--output_mlp_width", type=int, default=16, help="Width of output MLP"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--train_batch_size", type=int, default=1024, help="Batch size for training"
    )
    parser.add_argument(
        "--test_batch_size", type=int, default=8192, help="Batch size for testing"
    )
    parser.add_argument(
